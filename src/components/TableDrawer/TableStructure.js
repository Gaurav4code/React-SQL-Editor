  ...
  return (
    <div className="mx-10">
      <div className="flex items-center cursor-pointer" onClick={handleTable}>
        ...
        <p className="ml-3 text-lg font-bold text-gray-500">{tableName}[-]</p>
      </div>
      {tableHead.map((row, index) => (
        <div className="relative flex items-end ml-3" key={index}>
          <div className="w-10 h-8 border-b-2 border-l-2"></div>
          <p className="absolute text-sm font-semibold text-gray-500 top-5 left-9">
            {row}{" "}
